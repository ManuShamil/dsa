...
Node* BinaryTree :: insertNode(Node* currentPtr, int n) {
    if(currentPtr == NULL) {
        return createNode(n);
    }
    if(n < currentPtr->data) {
        currentPtr->left = insertNode(currentPtr->left, n);
    }
    if(n > currentPtr->data) {
        currentPtr->right = insertNode(currentPtr->right, n);
    }
    return currentPtr;
}
